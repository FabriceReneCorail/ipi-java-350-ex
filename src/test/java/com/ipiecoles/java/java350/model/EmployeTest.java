...
@Test 
public void testAugmenterSalaire(String matricule,Integer performance, Integer nbAnneesAnciennete, Double salaire, Double tempsPartiel) {
	//Given
	Employe employe= new Employe();
	employe.setMatricule(matricule);
	...
	employe.setTempsPartiel(tempsPartiel);
	//when
	Double augmentationSalaire = employe.getSalaire();
	//Then
	Assertions.assertThat(augmentationSalaire).isEqualTo(salaire);
 }

@Test
public void testGetNbRtt(Integer nbAnneesAnciennete, Double tempsPartiel) {
	//Given
	Employe employe = new Employe();
	employe.setDateEmbauche(LocalDate.now().minusYears(nbAnneesAnciennete));
	employe.setTempsPartiel(tempsPartiel);
